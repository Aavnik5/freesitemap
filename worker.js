...
const DOOD_API_KEY = "558416dy0311cdlc9k4mjd";
const DOOD_API_BASE = "https://doodapi.co/api/file/list";
// ============================================================
...
async function handleRequest(request) {
  ...
  // --- PART A.5: DOODSTREAM API FEED ---
  if (path === "/dood-feed") {
    const rawPage = parseInt(url.searchParams.get("page") || "1", 10);
    const rawPerPage = parseInt(url.searchParams.get("per_page") || "24", 10);
    const page = Number.isFinite(rawPage) && rawPage > 0 ? rawPage : 1;
    const perPage = Number.isFinite(rawPerPage) && rawPerPage > 0 ? Math.min(rawPerPage, 100) : 24;

    const doodUrl = `${DOOD_API_BASE}?key=${encodeURIComponent(DOOD_API_KEY)}&page=${page}&per_page=${perPage}`;

    try {
      const apiRes = await fetch(doodUrl, {
        headers: {
      ...
      return new Response(JSON.stringify({ status: 500, msg: err.message || "Dood fetch failed" }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders }
      });
    }
